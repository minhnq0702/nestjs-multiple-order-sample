...
run-ab-order:
	docker run --rm --read-only -v `pwd`:`pwd` -w `pwd` jordi/ab -T application/json -p ./ab-create-order.json -n 100 -c 25 -s 2 http://host.docker.internal:3000/api/orders/benchmark-over-qty
